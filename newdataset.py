...
def build_cls_split(det_base, cls_out):
    if os.path.exists(os.path.join(cls_out, "train", "fire")):
...
# PATHS & CONFIG
# ---------------------------------------------
CLASS_TRAIN     = f"{CLS_SPLIT_DIR}/train"
CLASS_VAL       = f"{CLS_SPLIT_DIR}/valid"
CLASS_TEST      = f"{CLS_SPLIT_DIR}/test"
...
print(f"DET_PRETRAINED : {DET_PRETRAINED}")
print(f"OUTPUT_DIR     : {OUTPUT_DIR}")
print(f"Device         : {'GPU' if torch.cuda.is_available() else 'CPU'}")
print(f"cls train/     : {os.path.exists(CLASS_TRAIN)}")
...
# ============================================================
print("\n[1/6] Fine-tuning Classification (100 epochs)...")
...
# ============================================================
print("\n[3/6] Fine-tuning Détection (100 epochs)...")
...
if conf_all:
    ...
    ax_dist.axvline(STRONG_THRESH, color="#7B0000", linestyle="--", linewidth=1.8, label=f"Strong = {STRONG_THRESH}")
    ax_dist.axvline(WEAK_THRESH,   color="#5D3800", linestyle="--", linewidth=1.8, label=f"Weak   = {WEAK_THRESH}")
    y_max = n_hist.max()
    ax_dist.text(0.15, y_max * 0.85, "Weak",   color="#5D3800", fontsize=11, fontweight="bold")
    ax_dist.text(0.43, y_max * 0.85, "Medium", color="#7A4200", fontsize=11, fontweight="bold")
...
legend_vis = [
    mpatches.Patch(color="#c0392b", label="Strong (>70%)"),
    mpatches.Patch(color="#e67e22", label="Medium (30-70%)"),
    mpatches.Patch(color="#f1c40f", label="Weak (<30%)"),
]
...
# ============================================================
# PARTIE F — BENCHMARK TIMING
# ============================================================
print("\n[6/6] Benchmark timing...")
