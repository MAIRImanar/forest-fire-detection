# ============================================================
#  APPROCHE YOLO11 ONLY — VERSION OPTIMALE
#  Classification : yolo11n-cls.pt   (Ultralytics officiel)
#  Detection      : best_nano_111.pt (pré-entraîné feu/fumée)
#  Split          : 70 / 15 / 15 manuel
...
BASE_OUT = find_real_path("MEMOIRE/Approche_YOLO11_Optimal_v2_20epochs")
...
if not os.path.exists(f"{SPLIT_DIR}/data.yaml"):
    print("Creation du split 70/15/15...")
    random.seed(42)
    ...
    print(f"Split deja existant : {SPLIT_DIR}")
...
print("=" * 60)
print("  APPROCHE YOLO11 ONLY — VERSION OPTIMALE")
print("  Classification : yolo11n-cls.pt")
print("  Detection      : best_nano_111.pt")
print("=" * 60)
print(f"DETECT_YAML : {DETECT_YAML}")
print(f"OUTPUT_DIR  : {OUTPUT_DIR}")
print(f"Device      : {'GPU' if torch.cuda.is_available() else 'CPU'}")
print(f"Split train : {os.path.exists(CLASS_TRAIN)}")
print(f"Split valid : {os.path.exists(CLASS_VAL)}")
print(f"Split test  : {os.path.exists(CLASS_TEST)}")

# ─────────────────────────────────────────────
# HELPER — intensite du feu selon confidence
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# 2. CHARGEMENT DU MODELE DE DETECTION
#    best_nano_111.pt — meilleur sur notre dataset
# ─────────────────────────────────────────────
print("\n[1/6] Chargement des modeles...")
...
if not os.path.exists(DET_PRETRAINED):
    ...
    print(f"  Modele copie vers Drive : {DET_PRETRAINED}")
else:
    print(f"  Modele deja disponible  : {DET_PRETRAINED}")

if not os.path.exists(DET_PRETRAINED) or os.path.getsize(DET_PRETRAINED) < 1000:
    raise FileNotFoundError(f"Modele introuvable : {DET_PRETRAINED}")

_tmp = YOLO(DET_PRETRAINED)
print(f"  Classes detection : {_tmp.names}  (nc={len(_tmp.names)})")
del _tmp

CLS_BASE_MODEL = "yolo11n-cls.pt"
print(f"  Classification    : {CLS_BASE_MODEL} (Ultralytics officiel)")
print(f"  Detection         : {DET_MODEL_NAME} ({os.path.getsize(DET_PRETRAINED)/1e6:.1f} MB)")


# ─────────────────────────────────────────────
# 3. CLASSIFICATION — FINE-TUNING
#    yolo11n-cls.pt  |  50 epochs  |  pas de freeze
# ─────────────────────────────────────────────
print("\n[2/6] Fine-tuning Classification (50 epochs)...")
...
# Test set manuel pour Precision / Recall / F1
all_samples    = []
...
for img_path, true_lbl in all_samples:
    ...
    preds_cls.append(int(res.probs.top1))
...
plt.show()
print("  Confusion matrix sauvegardee!")
...
#    best_nano_111.pt  |  50 epochs  |  pas de freeze
# ─────────────────────────────────────────────
print("\n[4/6] Fine-tuning Detection (50 epochs)...")
...
print(f"  best.pt : {best_det_path}")


# ─────────────────────────────────────────────
# 6. DETECTION — EVALUATION
# ─────────────────────────────────────────────
print("\n[5/6] Evaluation Detection...")
...
print("\n" + "="*50)
print("RAPPORT DETECTION YOLOv11")
print("="*50)
...
with open(os.path.join(OUTPUT_DIR, "yolo11_det_report.txt"), "w") as f:
    f.write("RAPPORT DETECTION YOLOv11\n" + "="*50 + "\n")
...
# Confusion Matrix Detection (2x4 : Ground Truth x Niveau confiance)
...
ax_cm2.set_title("Matrice de Confusion — Detection YOLOv11\nApproche YOLO11 Only (version optimale)",
...
if conf_all:
    ...
    ax_dist.axvline(STRONG_THRESH, color="#7B0000", linestyle="--", linewidth=1.8,
                    label=f"Seuil Strong = {STRONG_THRESH}")
    ax_dist.axvline(WEAK_THRESH,   color="#5D3800", linestyle="--", linewidth=1.8,
                    label=f"Seuil Weak   = {WEAK_THRESH}")
    y_max = n_hist.max()
    ax_dist.text(0.05, y_max * 0.85, "Weak Fire",   color="#5D3800", fontsize=11, fontweight="bold")
    ax_dist.text(0.35, y_max * 0.85, "Medium Fire", color="#7A4200", fontsize=11, fontweight="bold")
    ...
    ax_dist.set_title("Distribution des Confidence Scores — Detection YOLOv11",
...
# ─────────────────────────────────────────────
print("\n[6/6] Visualisation pipeline Classification → Detection...")
...
fig.suptitle("Pipeline YOLOv11 : Classification → Detection (Version Optimale)",
...
plt.show()
print("  Visualisations sauvegardees!")
...
print(f"Temps Detection      : {avg_det:.1f} ms")
...
print("\n" + "=" * 60)
print("  APPROCHE YOLO11 ONLY — VERSION OPTIMALE TERMINEE")
print("=" * 60)
